Shader "Custom/URP_OutlineNeon"
{
    Properties
    {
        ...
        [Toggle] _FirstMode ("First Mode (shape transparent, outline only, no noise)", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                if (_FirstMode < 0.5)
                {
                    // Режим с сохранением цвета фигуры: добавляем "неоновый" шум/пульсацию
                ...
                // else: firstMode == true -> чистая линия без шума
        ...
        // ---------- PASS 2: ОСНОВНАЯ ФИГУРА (сохраняет свой цвет) ----------
        Pass
        {
            Name "Main"
            Tags { "LightMode"="UniversalForward" }
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                // Если включен "первый режим" - фигура невидима, остаётся только обводка
                if (_FirstMode > 0.5)
                {
                    discard;
                }
